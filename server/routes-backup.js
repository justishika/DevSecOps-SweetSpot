...
export async function registerRoutes(app) {
    ...
    app.get("/api/vendor/products", isAuthenticated, async (req, res) => {
        try {
            ...
            // Status filter - by default exclude soft-deleted products
            if (status === "active") query.isActive = true;
            else if (status === "inactive") query.isActive = false;
            else {
                // Default: show only active products (exclude soft-deleted ones)
                query.isActive = true;
            }
            // Category filter
    ...
    app.get("/api/products", async (req, res) => {
        try {
            const { categoryId, vendorId, search, tags, dietary, isActive = "true" } = req.query;
            const query = {};
    ...
    app.delete("/api/products/:id", isAuthenticated, async (req, res) => {
        try {
            ...
            const product = await Product.findById(req.params.id);
            if (!product) return res.status(404).json({ message: "Product not found" });
            if (product.vendorId.toString() !== userId) return res.status(403).json({ message: "You can only delete your own products" });
    ...
    app.get("/api/orders", isAuthenticated, async (req, res) => {
        try {
            ...
            const orders = await Order.find(filters).populate('orderItems.product');
            res.json(orders);
        } catch (error) {
